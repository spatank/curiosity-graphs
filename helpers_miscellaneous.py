...
def average_area_under_the_curve(all_feature_values):
    """
    Returns the average area under the curve given a list of feature
...
def node_featurizer(graph_NX):
    ...
    for node in graph_NX.nodes():
        ...
        node_attributes = {'degree_1': graph_NX.degree(node),
                           'min_degree_1': min(neighborhood_degrees),
                           'max_degree_1': max(neighborhood_degrees),
                           'mean_degree_1': float(np.mean(neighborhood_degrees)),
                           'std_degree_1': float(np.std(neighborhood_degrees))}
